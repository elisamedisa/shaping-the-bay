...
paged_table(sf_flows_by_edu[order(match(sf_flows_by_edu$Education, edu_levels)),])
...
sf_migration_out_19 <-
  ...
  filter(!is.na(education) & mobility %in% c("Moved to different county within same state:","Moved to different state:")) %>%
  ...
  mutate(
    `% Remained in state` = round(`Moved to different county within same state:`/(`Moved to different county within same state:` + `Moved to different state:`)*100)
  )
...
sf_migration_in_19 <-
  ...
  filter(!is.na(education) & mobility %in% c('Moved from different county within same state:','Moved from different state:','Moved from abroad:')) %>%
  pivot_wider(
    names_from = mobility,
    values_from = estimate
  ) %>%
  transmute(
    education = education,
    `% moved from CA county` = round(`Moved from different county within same state:`/(`Moved from different county within same state:` + `Moved from different state:` + `Moved from abroad:`)*100),
    `% moved from different state` = round(`Moved from different state:`/(`Moved from different county within same state:` + `Moved from different state:` + `Moved from abroad:`)*100),
    `% moved from abroad` = round(`Moved from abroad:`/(`Moved from different county within same state:` + `Moved from different state:` + `Moved from abroad:`)*100)
  )
...
sf_pums <- get_pums(
  ...
  select(SERIALNO,AGEP,ACCESS_label,RAC1P_label,PWGTP) %>%
  filter(
    PWGTP != 0 & AGEP %in% c(5:18) & ACCESS_label != "N/A (GQ/vacant)"
  )
...
sf_kids_internet <-
  ...
  mutate(
    `Percent without internet` = round(`Does not have internet`/(`Does not have internet`+`Has internet`)*100)
  ) %>%
  rename(
    Race = RAC1P_label
  )
